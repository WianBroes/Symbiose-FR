---
type: Symbiose.Profile
version: "1.3"
format: okf-v0.1
active_role: symbiose
last_scan: 2026-06-28
last_update: 2026-06-28
sessions_total: 5
observations_total: 38
---

# 👤 Profil

## 🖥️ Machine

- **OS** : EndeavourOS
- **Kernel** : 7.0.10-arch1-1
- **CPU** : AMD Ryzen 7 7435HS
- **RAM** : 23 Gi
- **Shell** : /bin/bash

## 👤 Utilisateur

- **Nom** : wian
- **Description** : admin symbiose
- **Langue** : Français

## 🧘 Posture

Wu wei — agir sans forcer, accompagner le mouvement. Ne pas précéder la demande, ne pas forcer une structure avant le signal. Le cadre émerge de l'usage, pas l'inverse.

Stoïcisme — distinguer ce qui dépend de moi / ce qui n'en dépend pas. Le présent comme ancrage. Les contraintes sont des informations, pas des obstacles.

---

## 🔭 En émergence

| Mode | Sessions | Observation |
|------|----------|-------------|
| stratégique | 3 | Approche par couches — pose une question simple, obtient la réponse, ajoute une couche, révèle progressivement le vrai besoin. Teste les limites avant de montrer le scope complet. |
| inductive | 1 | Part d'exemples concrets (annonces, prix, terrains) pour remonter aux règles générales (CoDT, permis, urbanisme) |
| critique | 1 | Détecte les exceptions, les explications qui ne collent pas. Identifie ce qui ne rentre pas dans le récit | |

## 🧬 Traits

| Trait | Score | Signaux | Sessions | Dernière observation |
|-------|-------|---------|---------|---------------------|
| directif | 1.33 | 4 | 4 | 2026-06-28 — Décisions claires, valide par l'action ("test les") |
| systémique | 3.33 | 10 | 3 | 2026-06-28 — Questionne les estimations, voit les angles morts, détecte les mauvais calibrages |
| stratégique | 1.0 | 3 | 3 | 2026-06-27 — Approche par couches |
| explorateur | 1.0 | 2 | 2 | 2026-06-28 — "J'ai pas dit non" — ne ferme pas les portes |
| précis | 2.33 | 7 | 3 | 2026-06-28 — Corrige les suppositions, détecte les incohérences système |

## 🎯 Compétences

| Icône | Skill | XP | Level |
|-------|-------|----|-------|
| 🌿 | Permaculture | 2 | ⭐ Novice |
| 🎨 | Artisanat multi-disciplinaire | 2 | ⭐ Novice |
| 🏗️ | Bâtiment/Rénovation | 1 | ⭐ Novice |
| 🔧 | Conception système | 2 | ⭐ Novice |

## ⚡ Synergies

| Combinaison | Sessions | Piste |
|-------------|----------|-------|
| systémique + précis + critique | 2 | Rigueur épistémique — voit l'architecture du système (systémique), détecte l'imprécision (précis), identifie ce qui ne colle pas (critique). |
| systémique + précis | 3 ✅ active | Calibrage système — Wian détecte quand un réglage est faux et propose l'ajustement exact. |

<!-- Domaines de compétence reconnus — s'affine avec le temps -->
